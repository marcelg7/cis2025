    ...
    <div class="px-4 py-5 sm:p-6">
        ...
        <div class="mt-3 text-sm text-gray-500">
            ...
            @if ($contract->manufacturer || $contract->model || $contract->version || $contract->device_storage || $contract->extra_info)
                <p class="mt-1"><span class="font-medium">Device:</span> {{ implode(', ', array_filter([
                    $contract->manufacturer,
                    $contract->model,
                    $contract->version,
                    $contract->device_storage,
                    $contract->extra_info,
                ])) }}</p>
            @endif
        </div>
        <div class="mt-4 pt-3 border-t border-gray-200 flex items-center space-x-2">
            <div class="flex-shrink-0">
                <x-info-link href="{{ route('contracts.view', $contract->id) }}" class="inline-flex justify-center items-center w-10 h-10 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-blue-500" style="width: 40px; height: 40px; border-radius: 50%; background-color: #dbeafe; min-width: 40px; min-height: 40px; padding: 0;" title="View">
            ...
            @if ($contract->status === 'draft')
                <div class="flex-shrink-0">
                    <x-info-link href="{{ route('contracts.edit', $contract->id) }}" class="inline-flex justify-center items-center w-10 h-10 rounded-full bg-yellow-100 hover:bg-yellow-200 text-yellow-800 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-yellow-500" style="width: 40px; height: 40px; border-radius: 50%; background-color: #fefcbf; min-width: 40px; min-height: 40px; padding: 0;" title="Edit">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            @endif
            <div class="flex-shrink-0">
                <a href="{{ route('contracts.download', $contract->id) }}" class="inline-flex justify-center items-center w-10 h-10 rounded-full bg-green-100 hover:bg-green-200 text-green-800 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-green-500" style="width: 40px; height: 40px; border-radius: 50%; background-color: #d1fae5; min-width: 40px; min-height: 40px; padding: 0;" {{ $contract->status !== 'finalized' ? 'aria-disabled="true" class="opacity-50 cursor-not-allowed"' : '' }} title="Download">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="flex-shrink-0">
                <form action="{{ route('contracts.email', $contract->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="inline-flex justify-center items-center w-10 h-10 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-blue-500" style="width: 40px; height: 40px; border-radius: 50%; background-color: #dbeafe; min-width: 40px; min-height: 40px; padding: 0;" {{ $contract->status !== 'finalized' ? 'disabled class="opacity-50 cursor-not-allowed"' : '' }} title="Email">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="flex-shrink-0">
                <form action="{{ route('contracts.ftp', $contract->id) }}" method="POST">
                    @csrf
                    <x-primary-button type="submit" class="inline-flex justify-center items-center w-10 h-10 rounded-full bg-purple-100 hover:bg-purple-200 text-purple-800 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-purple-500" style="width: 40px; height: 40px; border-radius: 50%; background-color: #ede9fe; min-width: 40px; min-height: 40px; padding: 0;" title="Vault">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
