...
<div class="max-w-4xl mx-auto sm:px-6 lg:px-8 py-6 px-2 page-container">
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-gray-900">Edit Template</h1>
        <p class="mt-1 text-sm text-gray-600">Update template configuration</p>
    </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $contractTemplate->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="2" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $contractTemplate->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if(auth()->user()->hasRole('admin'))
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="is_team_template" id="is_team_template" value="1" {{ old('is_team_template', $contractTemplate->is_team_template) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-md p-3">
                    <p class="text-sm text-blue-700">
                        <strong>Usage:</strong> This template has been used {{ $contractTemplate->use_count }} times
                    </p>
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select name="activity_type_id" id="activity_type_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach(\App\Models\ActivityType::orderBy('name')->get() as $activityType)
                            <option value="{{ $activityType->id }}" {{ old('activity_type_id', $contractTemplate->activity_type_id) == $activityType->id ? 'selected' : '' }}>
                                {{ $activityType->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="location_id" id="location_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach(\App\Models\Location::orderBy('name')->get() as $location)
                            <option value="{{ $location->id }}" {{ old('location_id', $contractTemplate->location_id) == $location->id ? 'selected' : '' }}>
                                {{ $location->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="bell_device_id" id="bell_device_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach(\App\Models\BellDevice::orderBy('name')->get() as $device)
                            <option value="{{ $device->id }}" {{ old('bell_device_id', $contractTemplate->bell_device_id) == $device->id ? 'selected' : '' }}>
                                {{ $device->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="rate_plan_id" id="rate_plan_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach(\App\Models\RatePlan::orderBy('name')->get() as $plan)
                            <option value="{{ $plan->id }}" {{ old('rate_plan_id', $contractTemplate->rate_plan_id) == $plan->id ? 'selected' : '' }}>
                                {{ $plan->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="commitment_period_id" id="commitment_period_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach(\App\Models\CommitmentPeriod::orderBy('length_months')->get() as $period)
                            <option value="{{ $period->id }}" {{ old('commitment_period_id', $contractTemplate->commitment_period_id) == $period->id ? 'selected' : '' }}>
                                {{ $period->name }}
                            </option>
                        @endforeach
                    </select>
