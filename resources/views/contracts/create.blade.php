                        ...
                        <div>
                            ...
                            <select name="tier_select"
                                ...
                                @foreach($tiers as $tier)
                                    <option value="{{ $tier }}" {{ $tier === 'Select' ? 'selected' : '' }}>{{ $tier }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
			...
			<div>
				...
				<div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
					<p class="text-sm text-blue-900">
						...
						The plan cost from Bell pricing will be used for this contract.
					</p>
				</div>
			</div>
                        ...
                        @foreach (old('add_ons') as $index => $addOn)
                            <div class="add-on grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="add_ons[{{ $index }}][cost]" step="0.01" value="{{ old("add_ons.{$index}.cost") }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm total-input">
