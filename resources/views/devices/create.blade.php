...
@section('content')
	<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6 px-2"> <!-- Added px-2 -->
        ...
        <form method="POST" action="{{ route('devices.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
            @csrf
            <div>
                <label for="manufacturer" class="block text-sm font-medium text-gray-700">Manufacturer</label>
                <input type="text" name="manufacturer" id="manufacturer" value="{{ old('manufacturer') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                @error('manufacturer')
            ...
            <div>
                ...
                <input type="text" name="model" id="model" value="{{ old('model') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                @error('model')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input type="number" name="srp" id="srp" step="0.01" value="{{ old('srp') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                @error('srp')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div id="pricings" class="mt-4 space-y-4">
                    @foreach (old('pricings', $pricings) as $index => $pricing)
                        <div class="pricing grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="pricings[{{ $index }}][term]" min="0" value="{{ old("pricings.{$index}.term", $pricing['term']) }}" class="term-input mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" {{ old("pricings.{$index}.type", $pricing['type']) === 'smartpay' ? 'required' : '' }}>
    ...
    <script>
        let pricingCount = {{ count(old('pricings', $pricings)) }};
        function addPricing() {
